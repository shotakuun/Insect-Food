...
if defined?(AssetSync)
  AssetSync.configure do |config|
    config.enabled = false if Rails.env.development? # 本番環境アップロード
    config.fog_provider = 'AWS'
    config.fog_directory = 'insect-food'
    config.aws_access_key_id = ENV.fetch('S3_ACCESS_KEY_ID', nil)
    config.aws_secret_access_key = ENV.fetch('S3_SECRET_ACCESS_KEY', nil)
